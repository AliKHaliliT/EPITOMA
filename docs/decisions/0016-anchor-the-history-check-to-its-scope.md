# 0016. Anchor the history check to its scope
...
selection against the style's, rule by rule: every rule the style ships is present here, and the only setting this project holds that the style does not is the underscore exemption on unused variables, which permits rather than requires, so it left nothing in the tree to sweep. The two `_style` parameters in the preview styles depend on that exemption, and they keep the signature symmetry the renderers read.
...
Move the pin the attribution names.
