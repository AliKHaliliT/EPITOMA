# 0017. Drop the unused-parameter exemption
...
This tree held the only two uses in the family, the `_style` parameters on `entryHeaderStyle` and `sectionStyle` in the preview styles, which read nothing from the style and kept a symmetry no caller depends on: a grep over `src` and `tests` found four call sites, all naming the functions directly rather than dispatching over them, so the parameter stated a dependency that did not exist. Both parameters and all four arguments are gone. A parameter named `_style` planted in a scratch file drew
