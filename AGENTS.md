...
| `npm test` | Vitest suites for the service and the portfolio source, in a shuffled order under a seed the run prints |
...
that made it so and needs no repetition. The advisory checks here are the credential and
...
- **Read [docs/INVARIANTS.md](docs/INVARIANTS.md) before changing what the builder
...
- **The layer rule is absolute.** Imports point downward through
  ...
  [decision 0004, Build the builder as one-way sliced layers](docs/decisions/0004-build-the-builder-as-one-way-sliced-layers.md).
- **Content is checked at the door.** An uploaded snapshot passes `isPortfolioSnapshot`;
  ...
  [decision 0005, Check fetched seed files against the contract](docs/decisions/0005-check-fetched-seed-files-against-the-contract.md).
...
- **Nothing prints from product code**, and the linter gates the rule with the scripts
  ...
  diagnostic goes where the failure path already sends it, the typed error, the storage
  alert, or the notice that names an unreadable document store, never the console.
- **Suites mirror the source tree and substitute only at a seam.** One suite per unit under
  ...
  [decision 0009, Adopt the style's test contract](docs/decisions/0009-adopt-the-styles-test-contract.md).
...
repository, and the rulebook it lives under is owned there. The style stays one statement of
its form only if what a child learns flows back, so an improvement is never kept as a local
advantage, and the same road carries defects: a workaround of the template's own bytes or
prescribed behavior that nobody reports leaves every later child to hit it.

[docs/UPSTREAM.md](docs/UPSTREAM.md) is the one living document for this, present from
adoption on and registered in the index below. Its schema is fixed:

- The file opens with its title, one line naming the template and the commit this project
...
An entry names nothing that identifies this project, no project name, no person, no host, no
path that points at it, and no fact about its domain beyond what the entry needs, because the
file is handed to a public repository and may be quoted verbatim into its records.
...
style's records under [docs/inherited/](docs/inherited/) nor this project's own show it
...
plainly that the writer could not tell a defect from a misunderstanding, because the
maintainer decides that.
...
the new pin, the records the template gained, and the treasury's public dispositions: an entry
the template now carries is deleted and its form taken, an entry a record refuses is deleted
with this project conforming or turned into a decision record of its own where the matter is
this project's to decide, and an entry the template is silent on stays, re-verified and
re-dated once past its horizon, because silence for a season is the style's answer for now.

Re-alignment is one refactor, not a trickle. Read the decision records the template gained
since the pin, because every rule change carries one; recopy the files the style owns verbatim,
the rulebook, the docs audit and its selftest, the inherited records as one folder, and the
...
rule this configuration has that the style's does not, sweep out what that rule required,
since its absence upstream is a refusal rather than an oversight; resolve every open entry of
the upstream file as above; write the re-alignment as one decision record of this project's
own, which links every record the inherited folder gained, each with its title, and says for
each what in the tree it bound and what changed, or that it bound nothing and why, because
reading a record is not applying it, while the docs audit holds that every record the folder
gained is cited by a record of this project's own and leaves the honesty of each citation to
review; run every checking command above; and move the pin on the first line of that file. A
history-reading check binds this project from the commit its scope sentence arrived in, the
re-alignment commit itself, so a red gate over older commits is a defect to send upstream
rather than a reason to rewrite history. No changelog is kept, because the records are the
changelog and a summary would be a lossy copy of them.
...
| [docs/INVARIANTS.md](docs/INVARIANTS.md) | What must stay true about the builder, one row per claim with the holder that refuses a violation and the rung that says what its green is worth. Read before changing what the builder promises; a change to a claim changes its row. |
